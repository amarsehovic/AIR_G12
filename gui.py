...
class MovieRecommendationApp:
    def __init__(self, root):
        ...
        #USER SELECT
        self.label_user_id = tk.Label(self.left_frame, text="Select User ID:", anchor="w", width=label_width, fg=label_color) #select user id
        ...
        self.top_n_var = tk.IntVar(value=5)  #default value is 5
    ...
    def fetch_recommendations(self):
        ...
        if not user_id:
            messagebox.showwarning("No user selected", "Please select a user before proceeding!")
            return

        #recommendations label update based on the number of movies selected
        if top_n == 3:
        ...
        well_rated_movies = user_ratings[user_ratings['rating'] >= 4.0]
        ...
        if genre != "All Genres":
            genre_movies = self.movies[self.movies['genres'].str.contains(genre, regex=False)]
        else:
            genre_movies = self.movies

        unseen_movies = genre_movies[~genre_movies['movieId'].isin(well_rated_movie_ids)] #filter out movies the user has already rated
        unseen_movie_embeddings = {movie_id: self.movie_embeddings[movie_id] for movie_id in unseen_movies['movieId']}#get movie embeddings for unseen movies
        well_rated_movie_embeddings = {movie_id: self.movie_embeddings[movie_id] for movie_id in well_rated_movie_ids}#get embeddings
        recommendations = self.get_similar_movies(well_rated_movie_embeddings, unseen_movie_embeddings, top_n) #calc cosine similarity between wellrated and unseen movies


        #display recommendations
        self.listbox_recommendations.delete(0, tk.END)  #clear previous recs
    ...
    def get_similar_movies(self, well_rated_movie_embeddings, unseen_movie_embeddings, top_n):
        movie_similarities = []
        for unseen_movie_id, unseen_embedding in unseen_movie_embeddings.items(): #compare unseen movies with wellrated
